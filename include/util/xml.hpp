...
concept node_type = requires(const XML_NODE_CONTENT& node) {
...
concept single_node_content = std::convertible_to<NODE_CONTENT, std::string_view> || node_type<NODE_CONTENT>;

template<typename NODE_CONTENT>
concept node_content =
    single_node_content<NODE_CONTENT> ||
    (std::ranges::range<NODE_CONTENT> && single_node_content<std::ranges::range_value_t<NODE_CONTENT>>);

template<typename XML_NODE_CONTENT>
concept populated_node = node_type<XML_NODE_CONTENT> && requires(const XML_NODE_CONTENT& node) {
...
build(TYPE&& node)
{
    if constexpr (std::convertible_to<TYPE, std::string_view>) {
        ...
        return tag<TagType::START>(node) + build(node.content()) + tag<TagType::END>(node);
    } else {
...
struct node
{
    ...
    static constexpr auto attribute_names = AttrNames<sizeof...(ATTR_NAMES)>{ ATTR_NAMES... };
    static constexpr auto NO_ATTRIBUTE    = sizeof...(ATTR_NAMES);

    template<static_string ATTR>
        requires((ATTR == ATTR_NAMES) || ...)
    static constexpr auto index =
        []() { return std::distance(std::begin(attribute_names), std::ranges::find(attribute_names, ATTR)); }();
    ...
    constexpr auto xml() { return tag<TagType::EMPTY>(name, attribute_names, attribute_values); }
};
