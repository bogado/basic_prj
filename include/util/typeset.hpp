...
struct type_multi_set
{
    ...
    static constexpr auto index_of = []() {
        ...
        if constexpr (result.size() == 0) {
            ...
            auto is_same_filter = [](auto indice) { return map<T>[indice]; };
            std::ranges::copy(
                std::views::iota(std::size_t{ 0 }, size) | std::views::filter(is_same_filter), std::begin(result));
            return result;
        }
    }();
};
...
struct type_set<FIRST, ELEMENTS...>
{
    ...
    using as_optional =
        std::conditional<size == 1, std::optional<FIRST>, std::variant<std::monostate, FIRST, ELEMENTS...>>;
...
concept is_type_set = sizeof(std::pair<TYPESET, char>) == 1 && requires {
