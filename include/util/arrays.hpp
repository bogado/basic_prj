...
concept is_std_array =
    requires { requires std::tuple_size<T>::value == std::tuple_size_v<T>; };
...
constexpr auto constexpr_size_of = []() {
  ...
  if constexpr (std::is_bounded_array_v<TYPE>)
    return std::extent_v<TYPE>;
                       ...
                       requires std::integral<decltype(TYPE::extent)>;
                     })
    return TYPE::extent;
...
constexpr auto to_array(const ARRAY_LIKE_T array_like) {
  auto result = std::array<std::ranges::range_value_t<ARRAY_LIKE_T>,
...
static_assert(std::ranges::equal(array_of('a', 'b', 0),
                                 std::array{'a', 'b', '\0'}));

template <constexpr_sized FIRST_T, constexpr_sized... COLLECTION_Ts>
  requires(
      std::convertible_to<typename std::ranges::range_value_t<COLLECTION_Ts>,
                          std::ranges::range_value_t<FIRST_T>> &&
      ...)
consteval auto merge_arrays(const FIRST_T &first,
                            const COLLECTION_Ts &...collections) {
...
static_assert(std::ranges::equal(merge_arrays(std::array<char, 1>{'a'},
                                              std::array{'a', 'b', 'c'}),
                                 std::string_view("aabc")));
static_assert(std::ranges::equal(merge_arrays(std::array<char, 0>{}, "abc"),
                                 std::array{'a', 'b', 'c', '\0'}));
static_assert(merge_arrays(std::array<char, 0>{},
                           std::array<char, 1>{'a'})[0] == 'a');
