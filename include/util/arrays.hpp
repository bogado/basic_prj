...
template<typename T>
concept is_std_array = requires { requires std::tuple_size<T>::value == std::tuple_size_v<T>; };

static_assert(is_std_array<std::array<char, 3>>);
static_assert(is_std_array<std::array<char, 0>>);

template<typename T>
concept constexpr_sized = std::is_bounded_array_v<std::remove_cvref_t<T>> || requires { requires is_static_span<T>; } ||
...
constexpr auto
to_array(const ARRAY_LIKE_T array_like)
{
    auto result = std::array<std::ranges::range_value_t<ARRAY_LIKE_T>, constexpr_size_of<ARRAY_LIKE_T>>{};
...
constexpr auto
array_of(const T value, const OTHERS... others) -> std::array<T, sizeof...(OTHERS) + 1>
{
...
template<constexpr_sized FIRST_T, constexpr_sized... COLLECTION_Ts>
    requires(
        std::convertible_to<typename std::ranges::range_value_t<COLLECTION_Ts>, std::ranges::range_value_t<FIRST_T>> &&
        ...)
consteval auto
merge_arrays(const FIRST_T& first, const COLLECTION_Ts&...collections)
{
...
static_assert(std::ranges::equal(
    merge_arrays(std::array<char, 1>{ 'a' }, std::array{ 'a', 'b', 'c' }),
    std::string_view("aabc")));
