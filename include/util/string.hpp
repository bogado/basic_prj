...
struct static_string {
	...
	constexpr auto view() const noexcept
        -> string_view_type
    {
		return string_view_type{content.data(), size()};
	}
    ...
    constexpr auto substr(size_type start, difference_type len = length) const noexcept {
        ...
        return std::string_view{start_loc, std::next(start_loc, std::min<difference_type>(ssize() - static_cast<difference_type>(start), len))};
    }
    ...
    constexpr auto last() const noexcept {
        auto tail = content | std::views::reverse | std::views::drop_while([](auto chr) { return chr == '\0'; }) | std::views::take(1);
        return tail.front();
    }
    ...
    constexpr bool operator==(const static_string<OTHER_SIZE>&) const noexcept { return false; }

    template <size_type OTHER_SIZE>
    requires(OTHER_SIZE != LENGTH)
    constexpr bool operator!=(const static_string<OTHER_SIZE>&) const noexcept { return true; }
