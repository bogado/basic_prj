...
constexpr auto
throw_on_error(std::string_view name, INVOCABLE invocable, std::array<int, IGNORED_SIZE> ignored = {})
{
    return [=](ARGS... args, std::source_location source = std::source_location::current()) {
        ...
        auto throw_error = [&](auto err) -> result_t {
            ...
            throw std::system_error(
                code,
                std::string(name) + result_msg + " call_type::ERRNO = " + std::to_string(errno) + " ⇒ " +
                    code.message() + " at " + source.function_name() + " → " + source.file_name() + ":" +
...
struct Args
{
    ...
    void update_c()
    {
        c_data.clear();
        std::ranges::copy(
            data_source | std::views::all | std::views::transform([](auto& val) { return val.data(); }),
            std::back_inserter(c_data));
        c_data.push_back(nullptr);
    }

    auto get_c_pointer() const noexcept { return c_data.data(); }
    ...
    auto data() const noexcept { return c_data.data(); }
...
auto
exec(fs::path exe, is_arguments_type auto arguments, std::source_location source = std::source_location::current())
{
...
inline auto
pipe(std::source_location source = std::source_location::current()) -> std::array<int, 2>
{
    std::array<int, 2> result{ -1, -1 };
    throw_on_error<call_type::ERRNO>("pipe", [&result]() { return ::pipe(result.data()); })(source);
    return result;
}
...
inline auto
wait_pid(pid_t pid, int option = 0, std::source_location source = std::source_location::current()) -> status_type
{
    constexpr auto sys_waitpid =
        throw_on_error<call_type::ERRNO, pid_t, int *, int>("waitpid", ::waitpid, std::array{ EAGAIN });
    int status{ -1 };
...
inline auto
status_pid(pid_t pid, std::source_location source = std::source_location::current()) -> status_type
{
...
struct poll_arg
{
    ...
    operator ::pollfd() const { return pollfd{ fd, events, 0 }; }
};

template<std::same_as<poll_arg>... Ts>
auto
poll(std::chrono::milliseconds timeout, Ts... fd_s)
{
...
class spawn
{
    ...
    static constexpr auto posix_spawn{ throw_on_error<
        call_type::SPAWN,
        ::pid_t *,
        const char *,
        ...
        char *const *,
        char *const *>(
        std::string_view("spawnp").substr(0, LOOKUP == lookup::NO_LOOKUP ? 5 : 6),
        LOOKUP == lookup::NO_LOOKUP ? ::posix_spawn : ::posix_spawnp) };
    ...
    constexpr static auto spawn_file_actions_init{ throw_on_error<call_type::SPAWN, posix_spawn_file_actions_t *>(
        "posix_spawn_file_actions_init",
        ::posix_spawn_file_actions_init) };

    constexpr static auto spawn_file_actions_destroy{ throw_on_error<call_type::SPAWN, posix_spawn_file_actions_t *>(
        "posix_spawn_file_actions_destroy",
        ::posix_spawn_file_actions_destroy) };

    constexpr static auto spawn_file_actions_addclose{
        throw_on_error<call_type::SPAWN, posix_spawn_file_actions_t *, int>(
            "posix_spawn_file_actions_addclose",
            ::posix_spawn_file_actions_addclose)
    };

    constexpr static auto spawn_file_actions_adddup2{
        throw_on_error<call_type::SPAWN, posix_spawn_file_actions_t *, int, int>(
...
public:
    ...
    void cwd(fs::path dir) { work_directory = dir; }
