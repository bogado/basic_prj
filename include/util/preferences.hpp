...
private:
    ...
    constexpr auto fetch_short_option()
    {
        ...
        auto short_opt = description | std::views::drop_while([](auto chr) { return chr != short_option_prefix; }) |
    ...
    constexpr auto fecth_long_option()
    {
        ...
        return std::string_view(
            start, std::find_if_not(start, std::end(description), [](auto chr) { return isalnum(chr); }));
    }
};
